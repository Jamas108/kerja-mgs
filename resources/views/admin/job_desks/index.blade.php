...
@section('content')
    ...
    <div class="page-header d-flex flex-column flex-md-row align-items-md-center justify-content-between mb-4 gap-3">
        ...
        <div class="d-flex gap-2">
            <a href="{{ route('admin.manage_job_desks.create') }}" class="btn btn-primary">
            ...
            <ul class="nav nav-tabs px-4 pt-3" id="jobDeskTabs" role="tablist">
                ...
                <li class="nav-item" role="presentation">
                    <button class="nav-link" id="inprogress-tab" data-bs-toggle="tab" data-bs-target="#inprogress"
                        type="button" role="tab" aria-controls="inprogress" aria-selected="false">
                ...
                <li class="nav-item" role="presentation">
                    <button class="nav-link" id="notstarted-tab" data-bs-toggle="tab" data-bs-target="#notstarted"
                        type="button" role="tab" aria-controls="notstarted" aria-selected="false">
                                    ...
                                    @foreach ($jobDesks as $jobDesk)
                                        <tr>
                                            ...
                                            <td data-label="Tenggat Waktu">
                                                <span
                                                    class="badge {{ \Carbon\Carbon::parse($jobDesk->deadline)->isPast() ? 'bg-danger-light text-danger' : 'bg-primary-light text-primary' }}">
                                                        ...
                                                        @foreach ($jobDesk->assignments as $assignment)
                                                            <div class="d-flex align-items-center gap-2">
                                                                <div class="user-avatar"
                                                                    style="width: auto; height: 30px; font-size: 12px; padding-left:5px ; padding-right:5px">
                                                                    {{ $assignment->employee->name }}
                                                                </div>
                                                                <div class="user-avatar"
                                                                    style="width: auto; height: 30px; font-size: 12px; background-color:rgb(255, 202, 202); color: red;">
                                                                    {{ $assignment->employee->division->name }}
                                                                </div>
                                            ...
                                            <td data-label="Status">
                                                @if (
                                                    $jobDesk->assignments->where('status', 'final')->count() == $jobDesk->assignments->count() &&
                                                        $jobDesk->assignments->count() > 0)
                                                    <span class="badge bg-success-light text-success">
                                                        <i class="fas fa-check-circle me-1"></i>Selesai
                                                    </span>
                                                @elseif(
                                                    $jobDesk->assignments->whereIn('status', [
                                                            'assigned',
                                                            'completed',
                                                            'in_review_kadiv',
                                                            'kadiv_approved',
                                    ...
                                    <tr>
                                        <td colspan="7">
                                            <div class="alert alert-info d-flex align-items-center m-0" role="alert">
                                                ...
                                                <div>
                                                    <p class="mb-0">Belum ada tugas yang dibuat. Klik "Tambah Tugas Baru"
                                                        untuk memulai.</p>
                                                </div>
                                            </div>
                                        </td>
                                    ...
                                    <tr>
                                        <td colspan="7">
                                            <div class="alert alert-info d-flex align-items-center m-0" role="alert">
                                                ...
                                                <div>
                                                    <p class="mb-0">Tidak ada tugas yang sedang dalam proses saat ini.
                                                    </p>
                                                </div>
                                            </div>
                                        </td>
                        ...
                        <table class="table table-hover datatable-table" id="notstarted-tasks-table">
                            ...
                            <tbody>
                                ...
                                @foreach ($jobDesks as $jobDesk)
                                    @if (
                                        $jobDesk->assignments->count() == 0 ||
                                            $jobDesk->assignments->whereNotIn('status', [
                                                    'assigned',
                                                    'completed',
                                                    'in_review_kadiv',
                                                    'kadiv_approved',
                                                    ...
                                                    'rejected_director',
                                                    'final',
                                                ])->count() == $jobDesk->assignments->count())
        ...
        <script>
            $(document).ready(function() {
                ...
                $('.datatable-table').each(function() {
                    ...
                    //     responsive: true,
                    //     language: {
                    //         "lengthMenu": "Tampilkan _MENU_ entries per halaman",
                    //         "zeroRecords": "Data tidak ditemukan",
                    //         "info": "Menampilkan halaman _PAGE_ dari _PAGES_",
                    //         "infoEmpty": "Tidak ada data yang tersedia",
                    //         "infoFiltered": "(difilter dari _MAX_ total entries)",
                    //         "search": "Cari:",
                    //         "paginate": {
                    //             "first": "Pertama",
                    //             "last": "Terakhir",
            ...
            function confirmDelete(jobDeskId, jobDeskTitle) {
                ...
                deleteForm.action = "{{ route('admin.manage_job_desks.index') }}" + "/" + jobDeskId;
